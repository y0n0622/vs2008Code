// HappyNewYear.cpp : 定义控制台应用程序的入口点。
//
/********************************************************
*														*
*	程序名称：	新年祝福烟花								*
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	setcolor(RED);
	//outtextxy(300, 100, "C++项目练习 By FZY");
	//outtextxy(300, 200, "愿你悲伤有人分担！");
	//outtextxy(300, 280, "愿你孤独有人陪伴！");
	outtextxy(380, 200, "");
	outtextxy(580, 200, "");
	//getchar();
	Sleep(2000);
	...
	/*outtextxy(300, 50, "曾经");
	outtextxy(300, 100, "有一份真挚的爱情摆在我的面前");
	outtextxy(300, 150, "我没有珍惜");
	outtextxy(300, 200, "直到失去了才后悔莫及");
	outtextxy(300, 250, "世间最痛苦的事情莫过于此");
	outtextxy(300, 300, "如果上天能给我一个在来一次的机会");
	outtextxy(300, 350, "我会对那个女孩说");
	outtextxy(300, 400, "我爱你，一万年");
	outtextxy(300, 460, "——");*/
	...
	while (!kbhit())
	{
		...
		for (int clr = 0; clr < 1000; clr++)
		{
			for (int j = 0; j < 2; j++)
			{
				...
				if (py1 < 599)				// 防止越界
					pMem[py1 * 1100 + px1] = pMem[py1 * 1100 + px1 + 1] = BLACK;	// 对显存赋值擦出像素点
			}
		}
		Chose(t1);			// 筛选烟花
		Shoot();			// 发射烟花
...
void Style(DWORD& st1)
{
	...
	if (st2 - st1 >20000)		// 一首歌的时间
	{
		...
		outtextxy(260, 190, "我会好好照顾你");
		outtextxy(260, 250, "不让你受委屈");
		outtextxy(260, 310, "让你开心让你笑！");
		st1 = st2;
	}
			...
			for (double a = 0; a <= 6.28; a += 0.01)
			{
				...
				if (x1 > 0 && x1 < Fire[i].width && y1 > 0 && y1 < Fire[i].height)	// 只输出图片内的像素点
				{
					...
					if (r > 0x20 && g > 0x20 && b > 0x20 && xx > 0 && xx < 1100 && yy > 0 && yy < 600)
						pMem[yy * 1100 + xx] = BGR(Fire[i].xy[x1][y1]);	// 显存操作绘制烟花
				}
			}
			Fire[i].draw = false;
		}
